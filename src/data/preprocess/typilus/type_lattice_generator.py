...
class TypeLatticeGenerator:
    ...
    def __init__(
        ...
        # alias -> default name
        self.__aliases = {
            parse_type_annotation_node(k): parse_type_annotation_node(v) for k, v in rules["aliasing_rules"]
        }
        ...
        for k, v in rules["is_a_relations"]:
            self.__add_is_a_relationship(parse_type_annotation_node(k), parse_type_annotation_node(v))
    ...
    def __compute_non_generic_types(self):
        ...
        while len(to_visit) > 0:
            ...
            to_visit.extend((t for t in child_edges[next_node] if t not in generic_transitive_closure))
    ...
    def __add_is_a_relationship(self, from_type: TypeAnnotationNode, to_type: TypeAnnotationNode) -> None:
        ...
        if from_node_idx in all_reachable:
            self.__logger.warning(f"The {from_node_idx}<->{to_node_idx} would be a circle. Ignoring.")
            return
    ...
    def add_type(
        ...
        if annotation in self.__all_types:
            return
        pruned = annotation.accept_visitor(self.__max_depth_pruning_visitor, self.__max_annotation_depth)
    ...
    @lru_cache(16384)
    def __compute_erasures(self, type_annotation: TypeAnnotationNode) -> Tuple[List[TypeAnnotationNode], bool]:
    ...
    def build_graph(self):
        self.__logger.info("Building type graph for project... (%s elements to process)" % len(self.__to_process))
        i = 0
        ...
        while len(self.__to_process) > 0:
            ...
            if i > 500:
                ...
                if len(self.__to_process) > 3000:
                    self.__logger.info(f"Queue quite long. Current element {next_type}")
            ...
            all_inherited_types_and_self = next_type.accept_visitor(self.__direct_inheritance_rewriting)
        ...
        self.__logger.info("Done building type graph")

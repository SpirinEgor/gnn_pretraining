...
class AstGraphGenerator(NodeVisitor):
    ...
    def build(self):
        ...
        def parse_symbol_info(sinfo: SymbolInformation) -> Dict[str, Any]:
            ...
            if has_annotation:
                first_annotatable_location = min(k for k, v in sinfo.annotatable_locations.items() if v is not None)
        ...
        return {
            ...
            "edges": {
                e.name: {f: list(t) for f, t in v.items() if len(t) > 0} for e, v in self.__edges.items() if len(v) > 0
            },
    ...
    def __add_subtoken_of_edges(self):
        ...
        all_identifier_like_nodes: Set[TokenNode] = {n for n in self.__node_to_id if is_identifier_node(n)}
    ...
    def __visit_variable_like(
        ...
        if isinstance(name, Name):
            ...
            self.visit_Name_annotatable(name, lineno, col_offset, can_annotate_here, type_annotation)
            return
        name, node, symbol, symbol_type = self.__get_symbol_for_name(name, lineno, col_offset)

        if type_annotation is not None:
            type_annotation = self.__type_graph.canonicalize_annotation(type_annotation, self.__imported_symbols)
    ...
    def visit_alias(self, node: alias):
        if node.asname is not None:
            self.__imported_symbols[parse_type_annotation_node(node.asname)] = parse_type_annotation_node(node.name)
    ...
    def visit_Str(self, node: Str):
        self.add_terminal(TokenNode('"' + node.s + '"'))  # Approximate quote addition, but should be good enough.
